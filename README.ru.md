...
**На macOS и Linux хуки скорее всего промолчат.** В конфигурации указан `python`, а там бинарник называется `python3` — хук не запустится, и это не видно без проверки. Менять нужно **все вхождения**, а не первое: их десять на файл конфигурации, и если поправить один блок, журнал, счётчик повторов и подсказка при старте останутся молчать. Готовая команда — в [hooks/README.md](hooks/README.md#требования).
