...
Обновлять можно `hooks/*.py` и `skills/` — с одной оговоркой: если правил под проект `MEMORY_MATRIX`, `DB_CLIENTS`, `BROWSER_TESTER_WRITE_SEGMENTS` или константы зоны `QA_TESTER_*` в `guard.py`, сверь эти константы после копирования. Сам код специфики не содержит, а они содержат.
