...
const AREAS = [
  ...
  {
    key: 'tests-vs-code',
    doc: 'hooks/tests/*.py — то, что тесты утверждают своими именами и докстрингами',
    against: 'hooks/*.py — то, что код делает на самом деле',
    focus: 'зелёный тест при сломанной функции. Ищи: тест, который обходит проверяемое условие ' +
      'вместо того чтобы его создать (новый файл вместо правки существующего, новая команда ' +
      'вместо повтора той же); утверждение в имени или докстринге шире, чем то, что реально ' +
      'проверяется; ветка кода, до которой не доходит ни один тест. Оба бага, найденные ' +
      'прошлыми прогонами, жили именно здесь: тесты были зелёными',
  },
  {
    key: 'skills',
    doc: 'skills/*/SKILL.md',
