...
Пример подключения — hooks/README.md, раздел «Stop-хук: не завершать работу
с падающими тестами». В settings.example.json блока Stop нет намеренно: без
команды тестов хуку нечего запускать, поэтому он не включён по умолчанию.
