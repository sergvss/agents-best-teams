...
MESSAGES = {
    ...
    # Причину окна агент не видит: её показывают только человеку. Этот текст
    # приходит, когда человек разрешил, и когда окно показать некому - в
    # claude -p вызов становится отказом. На «нет» в окне Claude Code вложение
    # не доставляет: агент получает сообщение платформы остановиться и ждать.
    # Проверено в живой сессии; ветка «если отказал» здесь - для claude -p.
