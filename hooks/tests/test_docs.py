...
def anchor(title):
    """
    Якорь GitHub: нижний регистр, пунктуация выброшена, каждый пробел в дефис.

    Именно каждый, без склейки: из «встала — это» после удаления тире остаются
    два пробела, и GitHub даёт «встала--это». Со склейкой проверка признала бы
    такую ссылку битой и подтолкнула бы «чинить» правильную.
    """
